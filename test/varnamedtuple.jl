    ...
    @testset "Templated setindex $(c.vn)" begin
        ...
        vnt = if templated_unstable
            ...
            @test_throws ErrorException @inferred(
                AbstractPPL.templated_setindex!!(vnt_copy, c.val, c.vn, c.template)
            )
            AbstractPPL.templated_setindex!!(vnt, c.val, c.vn, c.template)
        else
            @inferred(AbstractPPL.templated_setindex!!(vnt, c.val, c.vn, c.template))
        end
...
@testset "VarNamedTuple" begin
    ...
    @testset "individual get/set" begin
        ...
        @testset "ComponentArray" begin
            ...
            for set_vn in vns
                vnt = AbstractPPL.templated_setindex!!(VarNamedTuple(), val, set_vn, ca)
                for get_vn in vns
                    @test vnt[get_vn] == val
                end
            end
            ...
            for (vn1, vn2) in
                ...
                vnt = VarNamedTuple()
                vnt = AbstractPPL.templated_setindex!!(vnt, val, vn1, ca)
                @test vnt[vn1] == vnt[vn2] == val # Sanity check.
                vnt = AbstractPPL.templated_setindex!!(vnt, new_val, vn2, ca)
                @test vnt[vn1] == vnt[vn2] == new_val
            end

            # Check that MustNotOverwrite is respected.
            for vn1 in vns
                vnt = AbstractPPL.templated_setindex!!(VarNamedTuple(), val, vn1, ca)
                for vn2 in vns
                    @test_throws MustNotOverwriteError AbstractPPL.VarNamedTuples.templated_setindex_no_overwrite!!(
                        vnt, new_val, vn2, ca
                    )
                end
            end
    ...
    @testset "GrowableArray behaviour" begin
        @testset "similar" begin
            ga = GrowableArray(ones(2, 3))
            @test similar(ga, (3, 4)) isa GrowableArray
            @test similar(ga, axes(ga)) isa GrowableArray
        end
    ...
    @testset "skeleton" begin
        function test_skeleton(orig_vnt, expected_skeleton)
            ...
            for (vn, val) in pairs(orig_vnt)
                ...
                template = get(skel.data, top_sym, AbstractPPL.NoTemplate())
                new_vnt = AbstractPPL.templated_setindex!!(new_vnt, val, vn, template)
            end
