...
function run_shared_gradient_tests(
    ...
    @testset "gradient path" begin
        ...
        @test AbstractPPL.capabilities(prepared) >= AbstractPPL.DerivativeCapability{1}()
...
`x0` is the prototype (zeros(3)), `xj = [2.0, 3.0, 4.0]` is the test point.
"""
function run_shared_jacobian_tests(
    ...
    @testset "jacobian path" begin
        ...
        @test cap == AbstractPPL.DerivativeCapability{1}(:vector)
        @test cap >= AbstractPPL.DerivativeCapability{1}()
        @test AbstractPPL.dimension(prepared) == length(x0)
