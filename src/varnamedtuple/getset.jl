...
"""
    AbstractPPL.VarNamedTuples._getindex_optic(
        collection, optic::AbstractPPL.AbstractOptic, orig_vn::VarName
    )
    AbstractPPL.VarNamedTuples._getindex_optic(collection, vn::VarName)
    ...
    sub_value = if optic.child isa AbstractPPL.Iden
        ...
        if Base.haskey(pa, coptic.ix...; coptic.kw...)
            ...
            if any(view(pa.mask, coptic.ix...; coptic.kw...))
                ...
                #     vnt = VarNamedTuple()
                #     x = zeros(2)
                #     vnt = AbstractPPL.templated_setindex!!(vnt, 1.0, @varname(x[1:2][1]), x)
                #     vnt = AbstractPPL.templated_setindex!!(vnt, 2.0, @varname(x[1:2][2]), x)
                need_merge = true
            end
...
A lot of the complexity in the VarNamedTuple code comes from this function, so its branches
are documented inline. Read those comments before modifying it.
"""
...
function make_leaf(value, ::AbstractPPL.Index, template::Any)
    ...
    # with.
    return error(
        "VarNamedTuple currently does not support indexed values within any container that is not an `AbstractArray` (got type $(typeof(template))).",
    )
end
