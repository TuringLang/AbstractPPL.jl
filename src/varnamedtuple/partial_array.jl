...
function Base.similar(ga::GrowableArray, sz::Tuple{Vararg{Union{Integer,Base.OneTo}}})
    return GrowableArray(similar(ga.data, sz))
end
...
This representation lets `PartialArray` retain array slices without flattening their values.
...
function grow_to_indices!!(
    ...
    return throw(
        ArgumentError(
            ...
            " the VarNamedTuple to have no knowledge of the shape of `x`. To" *
...
function _warn_growable_array_extraction()
    @warn (
        ...
        " of the actual `AbstractArray` represented by the `VarNamedTuple`." *
...
function _warn_growable_array_creation(size)
    @warn (
        ...
        " `AbstractArray` represented by the `VarNamedTuple`.\n\n" *
