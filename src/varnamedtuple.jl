...
# Increase the skip level when a caller prefixes the variable name; the template must be
# wrapped by the corresponding number of SkipTemplates.
...
function Base.showerror(io::IO, e::MustOverwriteError)
    ...
    return print(
        ...
        " AbstractPPL. Please file an issue at" *
        " https://github.com/TuringLang/AbstractPPL.jl/issues.",
    )
end
...
```jldoctest
julia> using AbstractPPL, BangBang
